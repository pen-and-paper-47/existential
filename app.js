...
function collectFinalData() {
    ...
    document.getElementById('pdf-name-label').innerText = currentLang === 'ru' ? "Идентификатор субъекта:" : "Subject Identifier:";
    document.getElementById('pdf-anxiety-label').innerText = currentLang === 'ru' ? "Уровень тревожности (0-10):" : "Anxiety Level (0-10):";
    document.getElementById('pdf-payment-label').innerText = currentLang === 'ru' ? "Метод списания премии:" : "Premium Deduction Method:";
    document.getElementById('pdf-risks-header').innerText = currentLang === 'ru' ? "Объекты покрытия:" : "Coverage Objects:";
    document.getElementById('pdf-signature-label').innerText = dict['pdf-signature-label'] || (currentLang === 'ru' ? "Подпись художника" : "Artist's signature");
    document.getElementById('pdf-footer-text').innerText = currentLang === 'ru' 
        ? "Данный документ подтверждает защиту от экзистенциальных угроз в рамках симуляции. Сгенерировано системой Алеф-404." 
        : "This document confirms protection against existential threats within the simulation. Generated by the Aleph-404 system.";
    ...
    document.getElementById('pdf-issue-date-label').innerText = currentLang === 'ru' ? "Дата выдачи" : "Date issued";
    document.getElementById('pdf-seal-text').innerText = currentLang === 'ru' ? "ВЕЧНЫЙ АРХИВ" : "ETERNAL ARCHIVE";

    const policyNumber = 'ALEPH-' + new Date().getFullYear() + '-' + Math.floor(100000 + Math.random() * 900000);
    document.getElementById('pdf-policy-number').innerText = policyNumber;

    const issueDateStr = new Date().toLocaleDateString(currentLang === 'ru' ? 'ru-RU' : 'en-GB', {
        day: '2-digit', month: '2-digit', year: 'numeric'
    });
    document.getElementById('pdf-issue-date').innerText = issueDateStr;

    // Генерируем полоски "штрихкода" внизу полиса
    const barcodeEl = document.getElementById('pdf-barcode');
    if (barcodeEl) {
        barcodeEl.innerHTML = '';
        for (let i = 0; i < 48; i++) {
            const bar = document.createElement('span');
            const isThick = Math.random() > 0.6;
            bar.style.flex = isThick ? '2' : '1';
            bar.style.opacity = Math.random() > 0.25 ? '1' : '0';
    ...
    // Метка "Итого:" / "Total:"
    const costLabelEl = document.getElementById('pdf-cost-label');
    if (costLabelEl) {
        costLabelEl.innerText = currentLang === 'ru' ? "Итого: " : "Total: ";
    }
    ...
    pdfElement.style.setProperty('background-color', '#f7f4ec', 'important');
    ...
    // Ждём загрузки шрифтов (PT Serif / Inter) и даём браузеру время отрисовать шаблон.
    Promise.all([
